...
function CheckboxIndicator({
  ...
  if (iconTheme === 'checkCircle' && showGlyph) {
    const FilledIndicator = indeterminate ? MinusCircleFilledIcon : CheckCircleFilledIcon;
...
const styles = StyleSheet.create({
  ...
  /** 与填充态 SVG 的圆等直径，并在 `indicatorSize` 槽位内居中，避免勾选前后尺寸跳变。 */
  circleIndicator: {
    width: tokens.circleIndicatorDiameter,
    height: tokens.circleIndicatorDiameter,
    borderWidth: tokens.glyphStrokeWidth,
    borderRadius: tokens.circleIndicatorDiameter / 2,
  },
